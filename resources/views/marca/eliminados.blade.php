        ...
        <div class="row mb-2">
            <div class="col-12 text-center">
                <div class="bg-azul-marino text-white rounded-3 px-3 py-2 d-inline-flex align-items-center gap-2 shadow-sm">
                    ...
                    <h1 class="h4 m-0">Marcas (eliminadas)</h1>
                </div>
            </div>
        </div>
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div>
                <a href="{{ route('marca.index') }}"
                   ...
                   title="Volver a marcas activas">
                    <i class="fa-solid fa-arrow-left"></i>
                </a>
            </div>

            <form method="GET" action="{{ route('marca.eliminados') }}"
                ...
                <div class="input-group">
                    ...
                    @if(request('q'))
                        <a href="{{ route('marca.eliminados') }}" class="btn btn-outline-secondary"
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle m-0">
                            ...
                            <tr>
                                ...
                                <td class="text-end">
                                    <div class="btn-group btn-group-sm" role="group">
                                        {{-- Restaurar --}}
                                        <form action="{{ route('marca.restaurar', $m->id) }}" method="POST" class="d-inline">
                                            @csrf @method('PATCH')
                                        ...
                                        <form action="{{ route('marca.forzar-eliminacion', $m->id) }}"
                                              method="POST" class="d-inline"
                                              onsubmit="return confirm('¿Eliminar PERMANENTEMENTE esta marca? Esta acción no se puede deshacer.');">
                                            @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="2" class="text-center py-4 text-muted">No hay marcas eliminadas.</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
